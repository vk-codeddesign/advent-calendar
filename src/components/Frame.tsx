...
export default function Frame({ blok, letter }: FrameComponentProps) {
  ...
  // Get current day of December, unlocking all after event year
  const today = new Date();
  const ADVENT_YEAR = 2024;
  const currentMonth = today.getMonth();
  const currentYear = today.getFullYear();
  let currentDay = 0;
  if (currentYear > ADVENT_YEAR) {
    // After the advent year, unlock all days
    currentDay = 31;
  ...
  return (
    <motion.div
      ...
      className={`overflow-hidden text-black border-0 bg-[#E9E9E6] transition-colors duration-500 ${
        !isSelected && !isAnimating && isAccessible && 'hover:scale-105'
        ...
        <AnimatePresence mode='wait'>
          {isSelected ? (
            <motion.div
              ...
              <motion.button layout className='absolute top-4 left-4' onClick={handleClose}>
                Close
              </motion.button>
              <motion.div
                layout
            ...
            <motion.h2
              ...
              className={`select-none text-fluid-6xl font-semibold ${
                (letter == 'x' || letter == 't') && 'text-fluid-9xl'
              } ${(letter == 'a' || letter == 'm' || letter == 'q') && 'text-fluid-8xl'}`}>
